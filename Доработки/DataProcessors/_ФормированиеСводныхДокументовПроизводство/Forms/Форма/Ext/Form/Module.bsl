...
&НаКлиенте
Процедура ЗаполнитьИСформировать(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВсеДокументыОтвет", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, "Заполнить и сформировать все документы?", РежимДиалогаВопрос.ДаНет, 60);
	
КонецПроцедуры

&НаКлиенте
Процедура ВсеДокументыОтвет(Ответ, ДополнительныеПараметры) Экспорт
	...
	ВыпускСформироватьДокументыОтвет(Ответ, ДополнительныеПараметры);
	ПередачаСформироватьДокументыОтвет(Ответ, ДополнительныеПараметры);
...
Процедура ВыпускСформироватьДокументыОтвет(Ответ, ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаТЗ из ТаблицаВыпускПродукции Цикл
		...
		Если СтрокаТЗ.Пометка Тогда
			...
			Состояние("" + СтрокаТЗ.Подразделение + " / " + СтрокаТЗ.Склад,, "Выпуск продукции");
...
Процедура ПередачаСформироватьДокументыОтвет(Ответ, ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаТЗ из ТаблицаПередачаВПроизводство Цикл
		...
		Если СтрокаТЗ.Пометка Тогда
			...
			Состояние("" + СтрокаТЗ.Подразделение + " / " + СтрокаТЗ.Склад,, "Передача материалов");
