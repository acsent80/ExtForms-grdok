...
Процедура Заполнить() Экспорт
	...
	Для каждого Строка1 из ТаблицаОстатков Цикл
		...
		Строка1.Длина   = ЗначенияСвойств.Длина;
		Строка1.Ширина  = ЗначенияСвойств.Ширина;
		Строка1.Толщина = ЗначенияСвойств.Толщина;
		
		Если НеВыгружатьКоличествоЛисты 
			И НоменклатураЛисты.НайтиПоЗначению(Строка1.Номенклатура) <> Неопределено Тогда
			Строка1.КоличествоЛистов = 100000;
		Иначе	
		...
		КонецЕсли;	
		Строка1.Наименование = ПолучитьНаименованиеНоменклатуры(Строка1.Номенклатура, Строка1.Характеристика, ЗначенияСвойств);
...
Функция ПолучитьНаименованиеНоменклатуры(Номенклатура, Характеристика, ЗначенияСвойств) Экспорт
	...
	Наименование = ЗначенияСвойств.Наименование + " " + Формат(ЗначенияСвойств.Радиус, "ЧГ=0") + ", " + СокрЛП(Покрытие);// + ", " + СокрЛП(Сорт);
