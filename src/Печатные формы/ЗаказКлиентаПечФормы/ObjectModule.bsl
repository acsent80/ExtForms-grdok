...
Функция СформироватьПечатнуюФорму(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	|	Док.Грузополучатель,
	|	Док.ГрафикОплаты,
	|	Док.Контрагент.ЮрФизЛицо КАК КонтрагентВид,
	|	Док.ДополнительнаяИнформация
	|ИЗ
	...
	|	ДокТовары.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения
	|ИЗ
	...
	|SWIFT CODE: [СчетБанкSWIFT]
	|Correspondent account number: [СчетКорСчет]";
	
	ШаблонБанкПосредник = 
	"Intermediary Bank: [СчетКорБанк], [СчетКорБанкАдрес].                       
	|SWIFT CODE: [СчетКорБанкSWIFT]";
	
	ШаблонБанкВТБ = 
	"57: Beneficiary Bank: 
	|SWIFT: [СчетБанкSWIFT]; Account with correspondent bank: [СчетКорСчет]; 
	|Name: [СчетБанк]; Address: [СчетБанкAddress]
	|56: Correspondent Bank of beneficiary’s bank:
	|SWIFT: [СчетКорБанкSWIFT]; National ID 50070010; Name: [СчетКорБанк]; 
	|Address: [СчетКорБанкАдрес]
	|59: Beneficiary:
	|Beneficiary’s name: [СчетБанк] Beneficiary’s account: [СчетНомерФилиала]
	|70: Payment details:
	|Beneficiary’s account: [СчетНомер]
	|Beneficiary’s name: [ОрганизацияEN]";
	
	ПервыйДокумент = Истина;
	Пока Шапка.Следующий() Цикл
		
		Если ИмяМакета = "ПФ_MXL_Заявка" Тогда
			
			Если Шапка.КонтрагентВид = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
				Макет = ПолучитьМакет(ИмяМакета);	
			Иначе
				Макет = ПолучитьМакет("ПФ_MXL_Заявка_RUS");	
			КонецЕсли;	
		...
		Попытка
			ОбластьБанкВТБ    = Макет.ПолучитьОбласть("БанкВТБ");
			ОбластьБанкПрочий = Макет.ПолучитьОбласть("Банк");
		Исключение
			ОбластьБанкВТБ    = Неопределено;
			ОбластьБанкПрочий = Неопределено;
		КонецПопытки;	
		...
		Если Шапка.Валюта.Код = "643" Тогда
			ДанныеШапки.Вставить("ВалютаEN", "RUB");
		Иначе	
			ДанныеШапки.Вставить("ВалютаEN", Шапка.Валюта);
		КонецЕсли;	
		ДанныеШапки.Вставить("ДатаОтгрузки",         Шапка.ДатаОтгрузки);
		ДанныеШапки.Вставить("ДатаОтгрузкиСтр",      Формат(Шапка.ДатаОтгрузки, "ДФ=dd.MM.yyyy"));
		ДанныеШапки.Вставить("ЖелаемаяДатаОтгрузки", Шапка.ЖелаемаяДатаОтгрузки);
		...
		ДанныеШапки.Вставить("СчетБанкSWIFT",                  СокрЛП(Шапка.СчетОрганизации.СВИФТБанка));
		...
		Если Шапка.СчетОрганизации.РучноеИзменениеРеквизитовБанка Тогда
			ДанныеШапки.Вставить("СчетКорСчет",                    Шапка.СчетОрганизации.СчетВБанкеДляРасчетов);
		Иначе
			ДанныеШапки.Вставить("СчетКорСчет",                    Шапка.СчетОрганизации.КоррСчетБанка);
		КонецЕсли;	
		ДанныеШапки.Вставить("СчетНомерФилиала",               _ОбщегоНазначенияДоп.ПолучитьДопРеквизит(Шапка.СчетОрганизации, "Beneficiary’s account"));
		...
		Если Пустаястрока(Шапка.ДополнительнаяИнформация) Тогда
			ДанныеШапки.Вставить("УсловияENG", "No special conditions");
			ДанныеШапки.Вставить("УсловияRUS", "Особые условия отсутствуют");
		Иначе	
			ДанныеШапки.Вставить("УсловияENG", Шапка.ДополнительнаяИнформация);
			ДанныеШапки.Вставить("УсловияRUS", Шапка.ДополнительнаяИнформация);
		КонецЕсли;	
		...
		Если НЕ ЗначениеЗаполнено(Шапка.Договор) Тогда
			...
			ДанныеШапки.Вставить("ДоговорДатаRUS",  "");
			ДанныеШапки.Вставить("ДоговорДатаENG",  "");
		...
		Иначе
			...
			ДанныеШапки.Вставить("ДоговорДатаRUS",  Формат(РеквизитыДоговора.Дата, "ДФ='dd.MM.yyyy'"));
			ДанныеШапки.Вставить("ДоговорДатаENG",  Формат(РеквизитыДоговора.Дата, "Л=en; ДФ='dd.MM.yyyy'"));
		...
		Если ВыборкаТовары.НайтиСледующий(Новый Структура("Ссылка", Шапка.Ссылка)) Тогда
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ЕдиницаВес = _ОбщегоНазначенияДоп.ПолучитьДопРеквизит(ВыборкаДетали.Характеристика, "Вес");
				Если НЕ ЗначениеЗаполнено(ЕдиницаВес) Тогда
					ЕдиницаВес = ВыборкаДетали.ЕдиницаВес;
				КонецЕсли;	
				
				ДанныеСтроки.Вставить("Нетто",     Окр(ВыборкаДетали.Количество * ЕдиницаВес, 0));
				ДанныеСтроки.Вставить("Брутто",    Окр(ВыборкаДетали.КоличествоМест * ВыборкаДетали.ЕдиницаМестВес, 0) + ДанныеСтроки.Нетто);
				...
				Если ЗначениеЗаполнено(ВыборкаДетали.ЕдиницаИзмерения.ЕдиницаИзмерения) Тогда
					ЕдиницаИзмерения = ВыборкаДетали.ЕдиницаИзмерения.ЕдиницаИзмерения;
				Иначе	
					ЕдиницаИзмерения = ВыборкаДетали.ЕдиницаИзмерения;
				КонецЕсли;	
					
				ДанныеСтроки.Вставить("ЕдИзм",     Строка(ЕдиницаИзмерения));
				ДанныеСтроки.Вставить("ЕдИзмEN",   Строка(ЕдиницаИзмерения.МеждународноеСокращение));
		...
		Если Шапка.СчетОрганизации.БИКБанка = "044525716" Тогда
			ОбластьБанк = ОбластьБанкВТБ;
		Иначе	
			ОбластьБанк = ОбластьБанкПрочий;
		КонецЕсли;	
		
		Если ОбластьБанк <> Неопределено Тогда
			ОбластьБанк.Параметры.Заполнить(ДанныеШапки);
			ТабДокумент.Вывести(ОбластьБанк);
		КонецЕсли;	
...
Процедура ЗаполнитьПараметрыКонтрагента(ДанныеШапки, Контрагент, ИмяРеквизита)
	...
	РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент, "НаименованиеПолное, РегистрационныйНомер, ИНН, КПП");
	...
	ДанныеШапки.Вставить(ИмяРеквизита + "ИНН",      РеквизитыКонтрагента.ИНН);
	ДанныеШапки.Вставить(ИмяРеквизита + "КПП",      РеквизитыКонтрагента.КПП);
