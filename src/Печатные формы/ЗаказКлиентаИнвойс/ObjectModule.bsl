...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ДобавитьКоманду(ТаблицаКоманд,
		"Invoice (EN)", // Представление команды в пользовательском интерфейсе
		"Инвойс_EN",	// Уникальный идентификатор команды
		);
...
Процедура Печать(МассивОбъектовНазначения, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Инвойс_EN") Тогда
		
		ТабличныйДокумент = ПечатьИнвойс_EN(МассивОбъектовНазначения, ОбъектыПечати, "ПФ_MXL_Инвойс_EN");
...
Функция ПечатьИнвойс_EN(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	ШаблонБанк = 
	"Beneficiary Bank: [СчетБанкENG]
	|[СчетБанкAddress].
	|Intermediary Bank: [СчетКорБанк],
	|[СчетКорБанкАдрес].
	...
	Пока Шапка.Следующий() Цикл
		...
		ДанныеШапки.Вставить("ОрганизацияАдресEN", ОрганизацияКИ["Адрес EN"]);
		...
		ДанныеШапки.Вставить("СчетБанкENG",                    Шапка.СчетОрганизации.НаименованиеБанка);
		...
		ДанныеШапки.Вставить("ОрганизацияБанкEN",              ЗаполнитьПоШаблону(ШаблонБанк, ДанныеШапки));
		...
		Иначе
			...
			РеквизитыДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Шапка.Договор, "Номер, Дата");
			ДанныеШапки.Вставить("ДоговорНомер", РеквизитыДоговора.Номер);
		...
		Если ВыборкаТовары.НайтиСледующий(Новый Структура("Ссылка", Шапка.Ссылка)) Тогда
			...
			Пока ВыборкаДетали.Следующий() Цикл
				...
				ДанныеСтроки.Вставить("ХарактеристикаEN",  ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаДетали.Характеристика, "НаименованиеПолное"));
				ДанныеСтроки.Вставить("НоменклатураХарактеристикаEN", ДанныеСтроки.НоменклатураEN + " (" + ДанныеСтроки.ХарактеристикаEN + ")");
...
Функция ЗаполнитьПоШаблону(Шаблон, Параметры)
