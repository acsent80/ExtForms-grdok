...
Функция ПолучитьДанныеДляПечатнойФормыТранспортнаяНакладнаяЛокальная(ТаблицаНакладных) 
	...
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная._ДатаПрибытия КАК ДатаПрибытия,
	...
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная._ФТСПунктПерехода.Описание КАК ФТСПунктПерехода,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная._ТаможняПокупателя.Наименование КАК ТаможняПокупателя,
...
Функция СформироватьПечатнуюФормуТранспортнойНакладной(МассивОбъектов, ОбъектыПечати, КомплектыПечати = Неопределено) 
	...
	ДвоичныеДанные = ПолучитьМакет("Картинка");		
	ТабличныйДокумент.ФиксированныйФон = Ложь;
	ТабличныйДокумент.ФоноваяКартинка = Новый Картинка(ДвоичныеДанные);		
...
Процедура ЗаполнитьТабличныйДокументТН(ТабличныйДокумент, СтруктураДанных, ОбъектыПечати, КомплектыПечати)
	...
	Для Каждого ДанныеПечати Из ТаблицаДанныхДляПечати Цикл
		...
		ОбластьМакета.Параметры.ДатаЗагрузки = Формат(ДанныеПечати.ДатаПрибытия, "ДФ=dd.MM.yyyy");
		ОбластьМакета.Параметры.ДатаОкончанияЗагрузки = Формат(ДанныеПечати.ДатаПрибытия, "ДФ=dd.MM.yyyy");
		...
		ОбластьМакета.Параметры.ФТСПунктПерехода  = ДанныеПечати.ФТСПунктПерехода;
		ОбластьМакета.Параметры.ТаможняПокупателя = ДанныеПечати.ТаможняПокупателя;
