...
Процедура ПриОткрытии(Отказ)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПроверкаФайлаЗавершение", ЭтотОбъект);
	
	ИмяФайлаError = КаталогВременныхФайлов() + "error.wav";
	Файл = Новый Файл(ИмяФайлаError);
...
Процедура ПроверкаФайлаЗавершение(Существует, ДопПараметры) Экспорт
	...
	ДвоичныеДанные = ПолучитьМакетErrorWav();
	ДвоичныеДанные.Записать(ИмяФайлаError);
	
КонецПроцедуры

&НаСервере
Функция ПолучитьМакетErrorWav()
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	Возврат ОбработкаОбъект.ПолучитьМакет("ErrorWav");
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	ТекущийШтрихкод = ДанныеШтрихкодов[0].Штрихкод;
	...
	Иначе
		флЕстьОшибка = Истина;
		ТекстОшибки = Новый ФорматированнаяСтрока("Штрихкод """ + ТекущийШтрихкод + """ не найден");
	КонецЕсли;	
...
Процедура ЗаполнитьСписокДокументов(Тип)
	...
	ИначеЕсли Тип = "РасходныйОрдерНаТовары" Тогда
		...
		Запрос.Параметры.Вставить("Статус", Перечисления.СтатусыРасходныхОрдеров.КПроверке);
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 10
		|	Док.Ссылка,
		|	Док.Номер,
...
Функция ПолучитьДанныеСерии(Серия)
	...
	Структура = Новый Структура("Номенклатура, Характеристика, Назначение, 
		|Серия, Документ, ДокументНомер, ДокументДата");
