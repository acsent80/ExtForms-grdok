...
Процедура НайтиОстаткиНаСервере()
	...
	Если ВсеОстатки Тогда
		...
		|					И Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК Рег) КАК Выборка
		|
		|СГРУППИРОВАТЬ ПО
		|	Выборка.Субконто1,
		|	Выборка.Субконто2
...
Процедура ПоказатьДокументыНаСервере(Субконто1, Субконто2, Подразделение)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Рег.Регистратор.Подразделение КАК Подразделение,
...
Процедура ОстаткиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ДеревоСубконто2" Тогда	
...
&НаСервере
Процедура ИсправитьПодразделениеНаСервере()
	...
	Для каждого СтрокаТЗ из ТаблицаДокументов Цикл
		...
		Для каждого Запись из НаборЗаписей Цикл
			...
			Если Запись.СчетДт = Объект.Счет
				И Запись.СубконтоДт[ВидСубконто1] = СтрокаТЗ.Контрагент
				И Запись.СубконтоДт[ВидСубконто2] = СтрокаТЗ.Договор Тогда
				Запись.ПодразделениеДт = Подразделение;
			КонецЕсли;	
			
			Если Запись.СчетКт = Объект.Счет
				И Запись.СубконтоКт[ВидСубконто1] = СтрокаТЗ.Контрагент
				И Запись.СубконтоКт[ВидСубконто2] = СтрокаТЗ.Договор Тогда
...
Процедура ИсправитьПодразделениеЗавершение(Ответ, ДопПараметры) Экспорт
	...
	ИсправитьПодразделениеНаСервере();
