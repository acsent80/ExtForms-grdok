...
Функция ОбработатьЭлемент(ДанныеЭлемента, ДополнительныеПараметры)
	...
	Разница = ДанныеЭлемента.КоличествоМатериал - ДанныеЭлемента.Количество;
	Если Разница > 0 Тогда
		КоличествоМатериал = Разница;
		КоличествоОтход    = 0;
	Иначе
		КоличествоМатериал = 0;
		КоличествоОтход    = -Разница;
	КонецЕсли;	
	
	СтруктураОтбора = Новый Структура("Номенклатура, Характеристика");
	НайденныеСтроки = ДокументОбъект.ВыходныеИзделия.НайтиСтроки(СтруктураОтбора);
	Если НайденныеСтроки.Количество() = 1 Тогда
		
		СтрокаТЧ = НайденныеСтроки[0];
		СтрокаТЧ.КоличествоФакт         = КоличествоОтход;
		СтрокаТЧ.КоличествоУпаковокФакт = КоличествоОтход;
		
		СтрокаТЧ.КоличествоОтклонение         = СтрокаТЧ.Количество - СтрокаТЧ.КоличествоФакт;
		СтрокаТЧ.КоличествоУпаковокОтклонение = СтрокаТЧ.КоличествоУпаковок - СтрокаТЧ.КоличествоУпаковокФакт;
	...
	Если НайденныеСтроки.Количество() = 1 Тогда
		...
		СтрокаТЧ = НайденныеСтроки[0];
		СтрокаТЧ.КоличествоФакт         = КоличествоМатериал;
		СтрокаТЧ.КоличествоУпаковокФакт = КоличествоМатериал;
