...
// В текстовой строке оставит только цифры
// здесь нужна что бы удалить разделитель из года
...
Функция ЗапросСписок()
	...
	|ИЗ
	|	втФизЛица КАК втФизЛица
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КлассыУсловийТрудаПоДолжностям.СрезПоследних() КАК КлассыУсловийТрудаПоДолжностям
...
Функция ПолучитьТекстФайлаНаСервере(ИмяФайлаХМЛ, Сообщения)
	...
	ОтвественныеЛица = ОтветственныеЛицаБП.ОтветственныеЛица(Организация, КонецКвартала(ОтчетныйПериод));
	
	ХМЛ.ЗаписатьНачалоЭлемента("Директор");
	
	ХМЛ.ЗаписатьНачалоЭлемента("ФИО"); 
	ХМЛ.ЗаписатьТекст(Строка(ОтвественныеЛица.Руководитель)); 
	ХМЛ.ЗаписатьКонецЭлемента();
	
	ХМЛ.ЗаписатьНачалоЭлемента("Должность"); 
	ХМЛ.ЗаписатьТекст(ОтвественныеЛица.РуководительДолжностьПредставление); 
	ХМЛ.ЗаписатьКонецЭлемента(); 
	...
	ХМЛ.ЗаписатьКонецЭлемента(); // Директор
	
	ХМЛ.ЗаписатьНачалоЭлемента("НачальникОтделаКадров");
	
	ХМЛ.ЗаписатьНачалоЭлемента("ФИО"); 
	ХМЛ.ЗаписатьТекст(Строка(ОтвественныеЛица.РуководительКадровойСлужбы)); 
	ХМЛ.ЗаписатьКонецЭлемента();
	
	ХМЛ.ЗаписатьНачалоЭлемента("Должность"); 
	ХМЛ.ЗаписатьТекст(ОтвественныеЛица.РуководительКадровойСлужбыДолжностьПредставление); 
	ХМЛ.ЗаписатьКонецЭлемента(); 
	...
	ХМЛ.ЗаписатьКонецЭлемента(); //НачальникОтделаКадров
