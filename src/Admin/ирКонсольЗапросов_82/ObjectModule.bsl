...
//  Массив – имен типов.
//
Функция ЛксПолучитьИменаПримитивныхТипов() Экспорт

	МассивТипов = Новый Массив;
	МассивТипов.Добавить("Null");
...
КонецФункции // ЛксПолучитьИменаПримитивныхТипов()
...
//  Ложь         – иначе.
//
Функция ЛксЛиКорневойТипСсылки(пИмяКорневогоТипа) Экспорт

	Если Ложь
		ИЛИ пИмяКорневогоТипа = "Перечисление"
		ИЛИ пИмяКорневогоТипа = "ТочкаМаршрутаБизнесПроцессаСсылка"
		ИЛИ ЛксЛиКорневойТипОбъектаБД(пИмяКорневогоТипа)
	Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
...
// Добавляет и заполняет новую строку в таблице реквизитов.
//
// Параметры:
...
//  Картинка     - Картинка.
//
Процедура ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, Имя, Синоним, Предопределенный, Картинка)

	ПолеРеквизита = ПостроительЗапроса.ДоступныеПоля.Найти(Имя);
	НовыйРеквизит = ТаблицаРеквизитов.Добавить();
	НовыйРеквизит.Имя = Имя;
	НовыйРеквизит.Синоним = Синоним;
	НовыйРеквизит.Предопределенный = Предопределенный;
	НовыйРеквизит.Тип = ПолеРеквизита.ТипЗначения;
	Если Картинка <> Неопределено Тогда
		НовыйРеквизит.Картинка = Картинка;
	КонецЕсли;

КонецПроцедуры // ЛксДобавитьВТаблицуРеквизитов()

// Получает таблицу реквизитов объекта БД.
//
// Параметры:
//  пОбъект      – определитель объекта метаданных;
//  *ЛиВключатьТолькоЧитаемые - Булево, *Ложь - включать ли в список только читаемые реквизиты;
...
//
// Возвращаемое значение:
//  ТаблицаРеквизитов – "Имя", "Представление", "Предопределенный", "Тип", "Картинка"*.
//
Функция ЛксПолучитьТаблицуРеквизитовОбъектаБД(пОбъект, ЛиВключатьТолькоЧитаемые = Ложь,
	ЛиВключатьНедоступные = Ложь, ЛиСортировать = Ложь, ЛиСКартинками = Ложь) Экспорт
	...
	ТаблицаРеквизитов.Колонки.Добавить("Предопределенный", Новый ОписаниеТипов("Булево"));
	ТаблицаРеквизитов.Колонки.Добавить("Тип", Новый ОписаниеТипов("ОписаниеТипов"));
	#Если Клиент Тогда
		Если ЛиСКартинками Тогда
			ТаблицаРеквизитов.Колонки.Добавить("Картинка");
		КонецЕсли;
	#КонецЕсли
	...
	МассивФрагментов = ЛксПолучитьМассивИзСтрокиСРазделителем(ПолноеИмяОбъекта);
	ПостроительЗапроса = Новый ПостроительЗапроса;
	ПостроительЗапроса.Текст = "ВЫБРАТЬ * ИЗ " + СтрЗаменить(ПолноеИмяОбъекта, ".ТабличнаяЧасть.", ".");
	ПостроительЗапроса.ЗаполнитьНастройки();
	Если МассивФрагментов.Количество() > 2 Тогда
		КорневойТип = "ТабличнаяЧасть";
	Иначе
	...
	#КонецЕсли
	Если КорневойТип = "Задача" Тогда
		ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "БизнесПроцесс", "Бизнес процесс", Истина, КартинкаРеквизита);
		ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Дата", "Дата", Истина, КартинкаРеквизита);
		Если ОбъектМетаданных.ДлинаНаименования > 0 Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Наименование", "Наименование", Истина, КартинкаРеквизита);
		КонецЕсли;
		Если ОбъектМетаданных.ДлинаНомера > 0 Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Номер", "Номер", Истина, КартинкаРеквизита);
		КонецЕсли;
		ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "ТочкаМаршрута", "Точка маршрута", Истина, КартинкаРеквизита);
	КонецЕсли;
	Если КорневойТип = "Документ" Тогда
		ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Дата", "Дата", Истина, КартинкаРеквизита);
		Если ОбъектМетаданных.ДлинаНомера > 0 Тогда
		...
		КонецЕсли;
		Если ЛиВключатьТолькоЧитаемые Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Проведен", "Проведен", Истина, КартинкаРеквизита);
		КонецЕсли;
	КонецЕсли;
	Если КорневойТип = "Справочник" Тогда
		Если ОбъектМетаданных.Владельцы.Количество() > 0 Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Владелец", "Владелец", Истина, КартинкаРеквизита);
		КонецЕсли;
	КонецЕсли;
	ЭтоГруппа = Ложь;
	Если ЛксЛиКорневойТипОбъектаСКодом(КорневойТип) Тогда
		Если ОбъектМетаданных.ДлинаКода > 0 Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Код", "Код", Истина, КартинкаРеквизита);
		КонецЕсли;
		Если ОбъектМетаданных.ДлинаНаименования > 0 Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Наименование", "Наименование", Истина, КартинкаРеквизита);
		КонецЕсли;
		Если ЛксЛиМетаданныеИерархическогоОбъекта(ОбъектМетаданных) Тогда 
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Родитель", "Родитель", Истина, КартинкаРеквизита);
			Если ЛксЛиМетаданныеОбъектаСГруппами(ОбъектМетаданных) Тогда
				ИерархияГрупп = Истина;
				Если Не ЛиВключатьНедоступные Тогда 
					ЭтоГруппа = пОбъект.ЭтоГруппа;
				КонецЕсли;
				Если ЛиВключатьТолькоЧитаемые Тогда
					ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "ЭтоГруппа", "Это группа", Истина, КартинкаРеквизита);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Если ЛксЛиКорневойТипОбъектаСПредопределенным(КорневойТип) Тогда
		Если ЛиВключатьТолькоЧитаемые Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Предопределенный", "Предопределенный", Истина,
				КартинкаРеквизита);
		КонецЕсли;
	КонецЕсли;
	Если ЛксЛиКорневойТипОбъектаБД(КорневойТип) Тогда
		ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "ПометкаУдаления", "Пометка удаления", Истина,
			КартинкаРеквизита);
		Если ЛиВключатьТолькоЧитаемые Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Ссылка", "Ссылка", Истина, КартинкаРеквизита);
		КонецЕсли;
	КонецЕсли;
	Если КорневойТип = "ТабличнаяЧасть" Тогда
		Если ЛиВключатьТолькоЧитаемые Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "НомерСтроки", "Номер строки", Истина, КартинкаРеквизита);
			// **** В случае ТабличнаяЧастьСтрока включать этот реквизит не нужно
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, "Ссылка", "Ссылка", Истина, КартинкаРеквизита);
		КонецЕсли; 
	КонецЕсли;
	#Если Клиент Тогда
	...
	Для Каждого МетаРеквизит Из ОбъектМетаданных.Реквизиты Цикл
		Если Ложь
			...
			Или (Истина
				...
				И МетаРеквизит.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента)
		Тогда
			ЛксДобавитьВТаблицуРеквизитов(ТаблицаРеквизитов, ПостроительЗапроса, МетаРеквизит.Имя, МетаРеквизит.Представление(), ,
				КартинкаРеквизита);
		КонецЕсли;
	КонецЦикла;
	Если ЛиСортировать Тогда
		ТаблицаРеквизитов.Сортировать("Представление");
	КонецЕсли;
	Возврат ТаблицаРеквизитов;

КонецФункции // ЛксПолучитьТаблицуРеквизитовОбъектаБД()
...
Функция ЛксСкопироватьУниверсальнуюКоллекцию(КоллекцияИсходная, КоллекцияПриемник = Неопределено) Экспорт
	...
	Если Ложь
		Или ТипЗнч(КоллекцияИсходная) = Тип("Массив")
		Или ТипЗнч(КоллекцияИсходная) = Тип("КоллекцияОбъектовМетаданных")
	Тогда
	...
	ИначеЕсли ТипЗнч(КоллекцияИсходная) = Тип("СписокЗначений") Тогда
		Если КоллекцияПриемник = Неопределено Тогда
...
//  Строка.
//
Функция ЛксПолучитьКонстантуТипаЗапроса(ТипПоля, ОписаниеТипов)

	Если ТипПоля = Тип("Строка") Тогда
		Результат = "ВЫРАЗИТЬ("""" КАК СТРОКА(" + ОписаниеТипов.КвалификаторыСтроки.Длина + "))";
	ИначеЕсли ТипПоля = Тип("Число") Тогда
		Результат = "ВЫРАЗИТЬ(0 КАК ЧИСЛО(" + ОписаниеТипов.КвалификаторыЧисла.Разрядность + ", " 
...
//  *ЛиЯзыкЗапросов - Булево, *Ложь - признак обработки ошибки при установке текста запроса;
...
Функция ЛксПоказатьОшибкуВЗапросеИлиПрограммномКоде(ПолеТекстовогоДокумента = Неопределено,
	СтартоваяСтрока = 0, СтартоваяКолонка = 0, ЛиЯзыкЗапросов = Ложь, ЛиМодально = Ложь, ИнформацияОбОшибке = Неопределено,
	ИмяМодуля = Неопределено) Экспорт
	...
	Если ПолеТекстовогоДокумента <> Неопределено Тогда
		...
		КонецЕсли;
		ТекстСообщения = ЛксПолучитьПредставлениеИзИдентификатора(ПолеТекстовогоДокумента.Имя) + ": " + ОписаниеОшибки;
	...
	Для ТекущаяКолонка = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
		...
		Для ТекущаяСтрока = НачальнаяСтрока По КонечнаяСтрока Цикл
			...
			Если Прогресс - СтарыйПрогресс >= 1 Тогда 
				...
				Состояние(СтрокаСостояния + Цел(100 * ПрогрессКолонок * ТекущаяСтрока) + "%");
			КонецЕсли;
			ШиринаКолонки = 0;
...
//
Функция ЛксПолучитьИндикаторПроцесса(Знач КоличествоПроходов, ПредставлениеПроцесса = "Выполнено", ВнутреннийСчетчик = Истина,
	Знач КоличествоОбновлений = 100, ЛиВыводитьВремя = Истина, РазрешитьПрерывание = Истина) Экспорт 
...
Функция ЛксПолучитьКнопкуКоманднойПанелиЭкземпляраКомпоненты(ОбъектКомпоненты, КраткоеИмяКнопки,
	...
	Возврат КоманднаяПанель.Кнопки.Найти(ПолноеИмяКнопки);
...
Процедура ЛксДобавитьКнопкиКоманднойПанелиКомпоненты(ОбъектКомпоненты, КнопкиМакета, КнопкаПриемника,
	...
	Для Каждого КнопкаМакета Из КнопкиМакета Цикл
		...
		Если КнопкаМакета.ТипКнопки = ТипКнопкиКоманднойПанели.Действие Тогда 
			Если Строка(КнопкаМакета.Действие) = "" Тогда
				// Это пустое действие
				Кнопка = КнопкиПриемника.Добавить(, ТипКнопкиКоманднойПанели.Действие);
				...
				КонецЕсли;
			Иначе
				Кнопка = КнопкиПриемника.Добавить(, ТипКнопкиКоманднойПанели.Действие, , КнопкаМакета.Действие);
				...
				КонецЕсли; 
				ЗаполнитьЗначенияСвойств(Кнопка, КнопкаМакета, , "ТипКнопки, Действие, Картинка");
			КонецЕсли; 
		КонецЕсли;
...
Процедура ЛксПодключитьОбработчикиСобытийДоступныхПолейКомпоновки(ТабличноеПоле) Экспорт 
	...
	// ++
	//ТабличноеПоле.Колонки.Заголовок.КартинкиСтрок = БиблиотекаКартинок.ирТипыДоступныхПолейКомпоновки;
	// --
...
//                 <продолжение описания параметра>.
//
Функция ЛксСоздатьСхемуПоТаблицамЗначений(СтруктураТаблиц, СхемаКомпоновкиДанных = Неопределено, СоздаватьПапкиПолей = Ложь) Экспорт 
	...
	Для Каждого КлючИЗначение Из СтруктураТаблиц Цикл
		...
		Для Каждого Колонка Из КлючИЗначение.Значение.Колонки Цикл
			...
			Если СоздаватьПапкиПолей Тогда
				ПутьКДанным = НаборДанных.Имя + "." + ПутьКДанным;
			КонецЕсли; 
...
//  Строка - для установки порядка.
//
Функция ЛксПолучитьСтрокуПорядкаКомпоновки(ПорядокКомпоновки) Экспорт
	...
	Для Каждого ЭлементПорядка Из ПорядокКомпоновки.Элементы Цикл
		...
		Тогда
			Продолжить;
		КонецЕсли;
		Строка = Строка + ", " + ЭлементПорядка.Поле + " ";
...
//
Процедура ЛксСравнитьСодержимоеЭлементаУправления(СравнительТабличныхДокументов, ЭлементУправления) Экспорт

	Если СравнительТабличныхДокументов = Неопределено Тогда
		СравнительТабличныхДокументов = Новый Массив;
	КонецЕсли;
	Если СравнительТабличныхДокументов.Количество() = 2 Тогда
		СравнительТабличныхДокументов.Удалить(0);
	КонецЕсли;
	Если ТипЗнч(ЭлементУправления) = Тип("ТабличноеПоле") Тогда
		...
		лПостроительОтчета.Вывести(ТабличныйДокумент);
	Иначе//Если ТипЗнч(ЭлементУправления) = Тип("ТабличноеПоле") Тогда
		ТабличныйДокумент = ЭлементУправления.ПолучитьОбласть();
	КонецЕсли;
	СравнительТабличныхДокументов.Добавить(ТабличныйДокумент);
	Если СравнительТабличныхДокументов.Количество() = 2 Тогда
		...
		Для Индекс = 0 По 1 Цикл
			...
			ТабличныйДокумент = СравнительТабличныхДокументов[Индекс]; 
		...
		Иначе
			Предупреждение("Табличные документы идентичны!");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЛксВвестиИУстановитьШиринуКолонокТабличногоПоля(ТабличноеПоле) Экспорт 
	...
	Если ВвестиЧисло(ВведенноеЗначениеШирины, "Введите ширину", 5, 0) Тогда
...
//
Процедура ЛксИнтерактивноУстановитьЗначениеВКолонкеТабличногоПоляТЧИлиТЗ(ТабличноеПоле, ЗначениеОбработки,
	ФормаИнициатор = Неопределено, Знач ТипИсточника = Неопределено, Знач Колонка = Неопределено,
	Знач ТолькоВыделенныеСтроки = Истина) Экспорт
	...
	Если ТипИсточника = Неопределено Тогда
		...
		Иначе
			Если ЛксПолучитьКорневойТипТабличнойЧасти(ТипЗначенияТабличногоПоля) <> Неопределено Тогда
	...
	Иначе
		Если ТипИсточника = "ТабличнаяЧасть" Тогда
			...
			ЛксСкопироватьОтборСтатический(Построитель.Отбор, ТабличноеПоле.ОтборСтрок, Истина);
	...
	Индикатор = ЛксПолучитьИндикаторПроцесса(КлючиСтрокДляОбработки.Количество());
	...
	Для Каждого КлючСтроки Из КлючиСтрокДляОбработки Цикл
		...
		ЛксИнтерактивноЗаписатьВКолонкуТабличногоПоля(ТабличноеПоле, Колонка, ЗначениеОбработки, ФормаИнициатор);
		ТабличноеПоле.ЗакончитьРедактированиеСтроки(Ложь);
	КонецЦикла; 
	
КонецПроцедуры // обИнтерактивноУстановитьЗначениеВКолонкеТабличногоПоляТЧИлиТЗ()
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ОБРАБОТКИ "Консоль Запросов"
...
Процедура ДобавитьПараметрыИзЗапроса(пЗапросОтладки, пТаблицаПараметров, ОписаниеТиповЭлементаУправленияПараметра)
	...
	Для каждого ПараметрЗапроса Из ПараметрыЗапроса Цикл
		...
		//Если ирПлатформа.ВерсияПлатформы >= 8001011 Тогда
			...
			Если пЗапросОтладки.Параметры.Свойство(ИмяПараметра, ЗначениеПараметраЗапроса) Тогда
				...
				Если ТипЗначенияПараметра = Тип("СписокЗначений") Тогда 
					СтрокаПараметров.ЗначениеПараметра = ЗначениеПараметраЗапроса;
				...
				Иначе
					...
					Сообщить("Тип """ + ТипЗначенияПараметра + """ значения параметра """ + ИмяПараметра + """ неотображаем в поле ввода",
						СтатусСообщения.Внимание);
					СтрокаПараметров.ЗначениеПараметра = "<Тип """ + ТипЗначенияПараметра + """ неотображаем>";
		...
		//		СтрокаПараметров.ЗначениеПараметра = РЗ[0][ИмяПараметра];
		//		Успех = Истина;
		//	Исключение
		//		ИнформацияОбОшибке = ИнформацияОбОшибке();
		...
		//			СтрокаПараметров.ЗначениеПараметра = М;
		//			СтрокаПараметров.ЭтоВыражение = 2;
		//			Успех = Истина;
		//		КонецЕсли; 
		//	КонецПопытки;
		...
		//			СтрокаПараметров.ЗначениеПараметра = "<Значение параметра """ + СтрокаПараметров.ИмяПараметра + """ неизвлекаемо>";
		//			СтрокаПараметров.НеИспользоватьОграничениеТипа = Истина;
		//		Иначе
		//			Сообщить("Значение параметра """ + ИмяПараметра + """ не задано", СтатусСообщения.Внимание);
		//			СтрокаПараметров.ЗначениеПараметра = "<Значение параметра """ + СтрокаПараметров.ИмяПараметра + """ не задано>";
		//		КонецЕсли;
		//		СтрокаПараметров.ИмяПараметра = "#" + СтрокаПараметров.ИмяПараметра;
		//	КонецЕсли;
		//КонецЕсли;
...
Функция ОткрытьДляОтладки(Запрос, ТипЗапроса = "Построитель", ИмяЗапроса = "Запрос для отладки", Модально = Истина) Экспорт
	...
	ДобавитьПараметрыИзЗапроса(мОбъектЗапроса, Форма.ЭлементыФормы.Параметры.Значение, 
		Форма.ЭлементыФормы.Параметры.Колонки.ЗначениеПараметра.ЭлементУправления.ТипЗначения);
...
Функция ОткрытьПоПостроителю(Построитель, ИмяЗапроса = "Исполняемый запрос построителя") Экспорт
	...
	//Если ирПлатформа.ВерсияПлатформы < 8001011 Тогда
		...
		Для Каждого ЭлементПараметра Из Построитель.Параметры Цикл
			...
			СтрокаПараметров.ЗначениеПараметра = ЭлементПараметра.Значение;
			Если ТипЗнч(СтрокаПараметров.ЗначениеПараметра) = Тип("СписокЗначений") Тогда 
				СтрокаПараметров.ЭтоВыражение = 2;
				СтрокаПараметров.ТипЗначения = СтрокаПараметров.ЗначениеПараметра.ТипЗначения;
			Иначе
				СтрокаПараметров.ЭтоВыражение = Ложь;
				СтрокаПараметров.ТипЗначения = Новый ОписаниеТипов(ЛксБыстрыйМассив(ТипЗнч(СтрокаПараметров.ЗначениеПараметра)));
			КонецЕсли;
		КонецЦикла;
...
Функция ОткрытьПоМакетуКомпоновки(МакетКомпоновки, Модально = Истина) Экспорт
	
	ИнициализацияСлужебногоРежима();
	мСтрокаЗапроса.ТипЗапроса = "Компоновка";
	Для Каждого ЗначениеПараметра Из МакетКомпоновки.ЗначенияПараметров Цикл
		СтрокаПараметров = мСтрокаЗапроса.ПараметрыЗапроса.Добавить();
		СтрокаПараметров.ИмяПараметра = ЗначениеПараметра.Имя;
		СтрокаПараметров.ЗначениеПараметра = ЗначениеПараметра.Значение;
		Если ТипЗнч(СтрокаПараметров.ЗначениеПараметра) = Тип("СписокЗначений") Тогда 
			СтрокаПараметров.ЭтоВыражение = 2;
			СтрокаПараметров.ТипЗначения = СтрокаПараметров.ЗначениеПараметра.ТипЗначения;
		Иначе
...
Функция РедактироватьНаборДанныхСхемыКомпоновкиДанных(ВладелецФормы, НаборДанных, Настройки) Экспорт
	...
	//	СтрокаПараметров = мСтрокаЗапроса.ПараметрыЗапроса.Добавить();
	//	СтрокаПараметров.ИмяПараметра = "" + ЗначениеПараметра.Параметр;
	//	СтрокаПараметров.ЗначениеПараметра = ЗначениеПараметра.Значение;
	//	Если ТипЗнч(СтрокаПараметров.ЗначениеПараметра) = Тип("СписокЗначений") Тогда 
	//		СтрокаПараметров.ЭтоВыражение = 2;
	//		СтрокаПараметров.ТипЗначения = СтрокаПараметров.ЗначениеПараметра.ТипЗначения;
