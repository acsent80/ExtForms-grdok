...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИспользоватьРежимМодальности() Тогда
		...
		ОбработкаВыбораЗавершение(Ответ, Параметр);
	Иначе
		...
		Оповещение = Вычислить("Новый ОписаниеОповещения(""ОбработкаВыбораЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаВыбораЗавершение(Ответ, Параметр) Экспорт
...
Процедура ОткрытьФормуРегистрацииНаУзлах(Команда)
	...
	Если Данные<>Неопределено Тогда
		...
		ОткрытьФорму(ПолучитьИмяФормы() + "Форма.УзлыРегистрацииОбъекта",
			Новый Структура("ОбъектРегистрации, ТаблицаРегистрации, ОповещатьОбИзменениях, Настройки", 
				Данные, ТаблицаРегистрации, Истина, Объект.Настройки
			),
			ЭтаФорма);
	КонецЕсли;
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Если ИспользоватьРежимМодальности() Тогда
		...
		РедактироватьНомерСообщенияКонстантыЗавершение(НомерСообщения, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""РедактироватьНомерСообщенияКонстантыЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		Выполнить("ПоказатьВводЧисла(Оповещение, НомерСообщения, Подсказка)");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура РедактироватьНомерСообщенияКонстантыЗавершение(НомерСообщения, Параметр) Экспорт
	Перем ТекДанные;
...
Процедура РедактироватьНомерСообщенияСсылки()
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
		ВвестиЧисло(НомерСообщения, Подсказка);
		РедактироватьНомерСообщенияСсылкиЗавершение(НомерСообщения, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""РедактироватьНомерСообщенияСсылкиЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		Выполнить("ПоказатьВводЧисла(Оповещение, НомерСообщения, Подсказка)");
	КонецЕсли;
...
&НаКлиенте
Процедура РедактироватьНомерСообщенияСсылкиЗавершение(НомерСообщения, Параметр) Экспорт
	Перем ТекДанные;
...
Процедура РедактироватьНомерСообщенияСписокНаборов()
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
		ВвестиЧисло(НомерСообщения, Подсказка);
		РедактироватьНомерСообщенияСписокНаборовЗавершение(НомерСообщения, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""РедактироватьНомерСообщенияСписокНаборовЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		Выполнить("ПоказатьВводЧисла(Оповещение, НомерСообщения, Подсказка)");
	КонецЕсли;
...
&НаКлиенте
Процедура РедактироватьНомерСообщенияСписокНаборовЗавершение(НомерСообщения, Параметр) Экспорт
	Перем ТекДанные;
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		УдалитьРегистрациюКонстантыВСпискеЗавершение(Ответ, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""УдалитьРегистрациюКонстантыВСпискеЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
	КонецЕсли;
...
&НаКлиенте
Процедура УдалитьРегистрациюКонстантыВСпискеЗавершение(Ответ, Параметр) Экспорт
...
Процедура УдалитьРегистрациюИзСпискаСсылок()
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		УдалитьРегистрациюИзСпискаСсылокЗавершение(Ответ, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""УдалитьРегистрациюИзСпискаСсылокЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
	КонецЕсли;
...
&НаКлиенте
Процедура УдалитьРегистрациюИзСпискаСсылокЗавершение(Ответ, Параметр) Экспорт
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации=Истина)
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		ДобавитьРегистрациюВыделенныхОбъектовЗавершение(Ответ, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""ДобавитьРегистрациюВыделенныхОбъектовЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьРегистрациюВыделенныхОбъектовЗавершение(Ответ, Параметр) Экспорт
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации=Истина)
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		УдалитьРегистрациюВыделенныхОбъектовЗавершение(Ответ, Параметр);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""УдалитьРегистрациюВыделенныхОбъектовЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
		ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
	КонецЕсли;
...
&НаКлиенте
Процедура УдалитьРегистрациюВыделенныхОбъектовЗавершение(Ответ, Параметр) Экспорт
...
Процедура ОбработкаВыбораДанных(ТаблицаФормы, ВыбранноеЗначение)
	...
	Иначе
		...
		Если ИспользоватьРежимМодальности() Тогда
			// Стандартно в модальном режиме (8.2/8.3) с обработкой результата.
			Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
			ОбработкаВыбораДанныхЗавершение(Ответ, Параметр);
		Иначе
			// Стандартно в немодальном режиме (8.3) с обработкой результата.
			//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
			Оповещение = Вычислить("Новый ОписаниеОповещения(""ОбработкаВыбораДанныхЗавершение"", ЭтаФормаЭтотОбъект(), Параметр)");
			ПоказатьВопрос(Оповещение, Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		КонецЕсли;
...
&НаКлиенте
Процедура ОбработкаВыбораДанныхЗавершение(Ответ, Параметр) Экспорт
...
Процедура ОткрытьФормуНастроекОбработки()
	...
	ОткрытьФорму(ТекИмяФормы, Новый Структура("Настройки", Объект.Настройки), ЭтаФорма);
КонецПроцедуры
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ИспользоватьРежимМодальности() Тогда
		...
		ОперацияСРезультатамиЗапросаЗавершение(Ответ, Неопределено);
	Иначе
		// Стандартно в немодальном режиме (8.3) с обработкой результата.
		//+yuraos, 10.09.2015 - для обратной совместимости лучше использовать атрибут "ЭтаФорма", "ЭтотОбъект" появляется в режиме совместимости "Версия 8.3.3" и выше.
		Оповещение = Вычислить("Новый ОписаниеОповещения(""ОперацияСРезультатамиЗапросаЗавершение"", ЭтаФормаЭтотОбъект(), Неопределено)");
...
&НаКлиенте
Процедура ОперацияСРезультатамиЗапросаЗавершение(Ответ, Параметр) Экспорт
	Перем ЗначениеОтвета;
