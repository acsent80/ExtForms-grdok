...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	//МассивНазначений.Добавить("Документ.ЗаказНаПроизводство");
	...
	ПараметрыРегистрации.Вставить("Наименование", "Удаление помеченных заявок на списание ДС");
	...
	ПараметрыРегистрации.Вставить("Информация", "Удаление помеченных заявок на списание ДС");
	
	// Создадим таблицу команд (подробнее смотрим ниже)
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	
	// Добавим команду в таблицу
	ДобавитьКоманду(ТаблицаКоманд, "Удаление помеченных заявок на списание ДС", "УдалениеПомеченныхЗаявокНаСписаниеДС", "ВызовСерверногоМетода", Истина, "");
...
Процедура ВыполнитьКоманду(ИдентификаторКоманды) Экспорт 
	...
	Если ИдентификаторКоманды = "УдалениеПомеченныхЗаявокНаСписаниеДС" Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств КАК Списание1
		|		ПО (Списание1.ЗаявкаНаРасходованиеДенежныхСредств = Док.Ссылка)
		|			И (НЕ Списание1.ПометкаУдаления)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств КАК Списание2
		|		ПО (Списание2.ДокументОснование = Док.Ссылка)
		|			И (НЕ Списание2.ПометкаУдаления)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК Списание3
		|		ПО (Списание3.ЗаявкаНаРасходованиеДенежныхСредств = Док.Ссылка)
		|			И (НЕ Списание3.Ссылка.ПометкаУдаления)
		|ГДЕ
		|	Док.ПометкаУдаления
		|	И Списание1.Ссылка ЕСТЬ NULL 
		|	И Списание2.Ссылка ЕСТЬ NULL 
		|	И Списание3.Ссылка ЕСТЬ NULL ";
		...
		ЗаписьЖурналаРегистрации("Удаление помеченных", 
			...
			"" + МассивПомеченных.Количество() + " документов");
			
		Для каждого ДокументСсылка из МассивПомеченных Цикл
