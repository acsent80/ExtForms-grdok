...
Процедура ПриОткрытии(Отказ)
	...
	ЭтоНужноеОкно = Ложь;
	Для каждого _Окно из МассивОкон Цикл
		Попытка
			ТекущаяФорма = _Окно.Содержимое[0];
			Если ТекущаяФорма.Объект.Ссылка = Параметры.ОбъектыНазначения[0] Тогда	
		...
		Если НЕ _Окно.Основное И ЭтоНужноеОкно Тогда
			Если ТипЗнч(ТекущаяФорма.ТекущийЭлемент) = Тип("ТаблицаФормы") Тогда 
				ТабличнаяЧасть = ТекущаяФорма.ТекущийЭлемент.Имя;
				Если ТипЗнч(ТекущаяФорма.ТекущийЭлемент.ТекущийЭлемент) = Тип("ПолеФормы") Тогда 
					Если ТекущаяФорма.ТекущийЭлемент.ТекущийЭлемент.Вид  = ВидПоляФОрмы.ПолеВвода Тогда
						л_Отказ = Ложь;
						ПолеИмяФормы = Параметры.ИмяФормы;
						ТекущееПоле = ТекущаяФорма.ТекущийЭлемент.ТекущийЭлемент.Имя;
						ПолеЗаполнения = СтрЗаменить(ТекущееПоле,ТабличнаяЧасть,"");
						ЗначениеЗаполнения = ТекущаяФорма.ТекущийЭлемент.ТекущиеДанные[ПолеЗаполнения];
